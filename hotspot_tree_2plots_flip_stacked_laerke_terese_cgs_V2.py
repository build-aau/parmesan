...
def generate_plot_stacked(data, conf_target_name, target_node_type, indicator, origin_node_id, origin_find_instance, origin_english_trans):
    ...
    # When user has given an input for origin_node_id, eg. 'Terrændæk'
    if target_node_type == 'ElementInstance':
        #plt.figtext(.5, .93, constants.hotspot_titles_all_building['element'] + " within " + origin_english_trans + " (Total)", fontsize=18, ha='center')
        plt.subplots_adjust(left=0.15, top=0.8)

    elif target_node_type == 'ConstructionInstance':
        #plt.figtext(.5, .93, constants.hotspot_titles_all_building['construction'] + " within " + origin_english_trans + " (Total)", fontsize=18, ha='center')
        plt.subplots_adjust(left=0.3, top=0.8)
    elif target_node_type == 'ProductInstance':
        #plt.figtext(.5, .93, constants.hotspot_titles_all_building['product'] + " within " + origin_english_trans + " (Total)", fontsize=18, ha='center')
        plt.subplots_adjust(left=0.3, top=0.8)
...
def analysis_target_res(data, conf_target_name, target_node_type, indicator, origin_node_id, origin_find_instance, input_stage):
    """
...
def generate_plot_target_res(data, conf_target_name, target_node_type, indicator, origin_node_id, origin_find_instance, origin_english_trans): #res_dict, x_values, y_values,
    # TODO: Make y-axis names dobbelt line
    # Calls the function analysis and gets the needed variables to produce the plot
    res_dict, x_values, y_values = analysis_stacked(data, conf_target_name, target_node_type, indicator, origin_node_id, origin_find_instance)
    ...
    width = 0.1  # Width of a group of bars
    x = np.arange(len(x_values)) # Label location

    # Generate indeces starting in 1 to represent label names along the x-axis.
    # (These are referenced int the dataframe plotted as a table nex tot the barplot.)
    num_range = range(1, (len(x)+1))
    ...
    if target_node_type == 'ElementInstance':
        plt.figtext(.5, .93, constants.hotspot_titles_all_building['element'] + " within " + origin_english_trans + " (Total)", fontsize=18, ha='center')
        plt.legend(bbox_to_anchor=(1.01, 1), loc='upper left', borderaxespad=0., title = "Sorted by: " + user_friendly_conf_target_name, prop = {"size": 6})

        plt.subplots_adjust(left=0.15, top=0.8)
    elif target_node_type == 'ConstructionInstance':
        plt.figtext(.5, .93, constants.hotspot_titles_all_building['construction'] + " within " + origin_english_trans + " (Total)", fontsize=18, ha='center')
        plt.legend(loc='upper right', title = "Sorted by: " + user_friendly_conf_target_name, fontsize= 8)
        plt.subplots_adjust(left=0.3, top=0.8)
    elif target_node_type == 'ProductInstance':
        plt.figtext(.5, .93, constants.hotspot_titles_all_building['product'] + " within " + origin_english_trans + " (Total)", fontsize=18, ha='center')
        plt.legend(loc='upper right', title = "Sorted by: " + user_friendly_conf_target_name)
        plt.subplots_adjust(left=0.3, top=0.8)
...
def user_input(data, input_stage, conf_target_name, indicator, target_node_type, origin_node_id, stacked_stages, target_stage, target_res):
    """
    ...
    if stacked_stages and target_stage is None and target_res is None and origin_node_id is None:
        ...
        '''
    #  LÆRKE/CHRISTAN HJÆLP - DEN PLOTTER IKKE DET KORREKTE - har prøvet med not none
    # NOT OK: Branch for ikke stacked stages, for the entire building, for the 10 worst instances for the given target_node_type
    ...
    #  LÆRKE/CHRISTAN HJÆLP - DEN PLOTTER IKKE DET KORREKTE
    # Branch for ikke stacked stages, for the entire building, for the 10 worst instances for the given target_node_type
    elif stacked_stages is False and target_res is None and target_stage is not None and origin_node_id:

        if target_node_type == 'ElementInstance':
            generate_plot_target_res(data, conf_target_name, target_node_type, indicator,
                                     origin_node_id=origin_node_id, origin_find_instance=True,
                                     origin_english_trans=None)
    ...
    # Branch for stacked stages, only for the entire building at the element level, for the
    # 10 worst instances for the given target_node_type
    # TODO: Exception: No node for the given node id is found 91329025-d117-4a64-a577-58bb1d76d82e
    elif stacked_stages and target_stage is None and target_res is None:
    ...
    # TODO: Lav dette færdig efter fået hjælp fra Christian
...
def main():
    ...
    stacked_stages = True  # True
    target_res = None #['Sum']
    target_stage = None #['A1to3'] None
    indicator = 'GWP'
    conf_target_name = 'conf_000000'
    origin_node_id = None #'91329025-d117-4a64-a577-58bb1d76d82e' #None #
    origin_english_trans = 'Roof construction'
    target_node_type = 'ProductInstance'
    user_input(data, target_stage, conf_target_name, indicator, target_node_type, origin_node_id, stacked_stages, target_stage, target_res)
